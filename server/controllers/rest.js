(function() {
    function CRUD_Controller(ModelName) {
        ...
        var process = function(req, res, all, model, funk, queryParams) {
            if (arguments > 6) {
                model[funk](queryParams, arguments[5], sendResult(req, res, all))
            } else {
                model[funk](queryParams, sendResult(req, res, all))
            }
        }

        function _fetch(req, res) { process(req, res, true, Model, 'find', {}) }
        function _fetchOne(req, res) { process(req, res, false, Model, 'findOne', {_id:req.params.id}) }
        function _create(req, res) { process(req, res, true, Model, 'create', req.body) }
        function _createFetchOne(req, res) { process(req, res, false, Model, 'create', req.body) }
       ...
       return {
            ...
            fetchOne: _fetchOne,
            create: _create,
            createFetchOne: _createFetchOne,
            update: _update,
            delete: _delete
        }
    }
